...
stateless control plane: register / unregister / discovery / channel
...
* ``GET    /channels/{channel_id}``        — get channel metadata
* ``POST   /channels/{channel_id}/close``  — close channel
* ``POST   /channels/{channel_id}/envelopes`` — post envelope
* ``GET    /channels/{channel_id}/wal``    — read WAL slice
...
def make_http_app(hub: "Hub") -> Starlette:
    ...
    async def create_channel(request: Request) -> Response:
        ...
        try:
            ...
            return _error_response(exc)
        return JSONResponse({"channel": metadata.to_dict()}, status_code=201)
    ...
    async def list_channels(request: Request) -> Response:
        ...
        try:
            channels = await hub.list_channels(agent_id=agent_id, limit=limit * 4)
        except NetworkError as exc:
            return _error_response(exc)
    ...
    async def get_channel(request: Request) -> Response:
        try:
            metadata = await hub.get_channel(request.path_params["channel_id"])
        except NetworkError as exc:
            return _error_response(exc)
        return JSONResponse({"channel": metadata.to_dict()})

    async def close_channel(request: Request) -> Response:
        body: dict[str, Any] = {}
        if int(request.headers.get("content-length", "0") or 0) > 0:
            body = await request.json()
        try:
            metadata = await hub.close_channel(
                request.path_params["channel_id"],
    ...
    async def post_envelope(request: Request) -> Response:
        ...
        envelope.channel_id = request.path_params["channel_id"]
        try:
    ...
    routes = [
        ...
        Route("/channels", create_channel, methods=["POST"]),
        Route("/channels", list_channels, methods=["GET"]),
        Route("/channels/{channel_id:str}", get_channel, methods=["GET"]),
        Route("/channels/{channel_id:str}/close", close_channel, methods=["POST"]),
        Route("/channels/{channel_id:str}/envelopes", post_envelope, methods=["POST"]),
        Route("/channels/{channel_id:str}/wal", read_wal, methods=["GET"]),
    ]
