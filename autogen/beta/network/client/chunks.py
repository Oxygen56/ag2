...
A ``ChunkDelta`` is the per-yield value of :meth:`Channel.iter_chunks`.
``ChunkSubscription`` owns the per-(channel, parent_envelope_id) queue
...
same agent on the same channel don't cross-talk.
"""
...
class ChunkSubscription:
    """Per-(channel, parent_envelope_id) queue.
