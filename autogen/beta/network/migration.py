...
def from_classic_pattern(
    ...
    *,
    ...
    Returns:
        ``(graph, tools)`` — the translated :class:`TransitionGraph`
        and a list of ``Handoff``-returning tools the caller should
        attach to the selector agent (``selector.tools.extend(tools)``).
        ``tools`` is empty for patterns that don't need a selector
        surface (``RoundRobinPattern``).
    ...
    if cls_name == "DefaultPattern":
        ...
        raise UnsupportedPatternError(
            "DefaultPattern → WorkflowGraph translation is planned "
            "(needs ContextExpr / OnContextCondition); for now express "
