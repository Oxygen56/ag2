...
class TurnWithinEvaluator:
    """Fire when an adapter's expected next speaker hasn't posted within
    T seconds of the trigger envelope.
    ...
    def evaluate(
        ...
        if trigger_iso is None:
            # No inbound trigger yet (e.g. channel just opened) — give
            # the expected speaker the same threshold from channel
...
class ProgressWithinEvaluator:
    """Fire when an active task has had no progress for T seconds.
...
class MinParticipationEvaluator:
    ...
    discussion-style channels where every voice should be heard.
...
class WarnHandler:
    """Audit + emit ``EV_EXPECTATION_VIOLATED`` to the violator(s) only.

    Differs from ``notify_channel`` (broadcast): ``warn`` is targeted,
    so the offending participant gets the signal without spamming the
    whole channel. Falls back to a broadcast for channel-wide
...
class HideHandler:
    ...
    closing the channel. In-memory only — a hub restart drops the
    flag.
    """
    ...
    async def handle(
        ...
        for vid in violation.violator_ids:
            hub.mark_hidden(channel_id, vid)


class RemoveHandler:
    """Audit + bar the violator from sending into the channel.

    Posts ``ag2.participant.removed`` with the offending agent_id and
    reason so peers can react. The bar is persisted to
    ``channels/{id}/removed.json`` and reapplied on hub restart.
    ...
    async def handle(
        ...
        if metadata is None or metadata.is_terminal():
            return
        for vid in violation.violator_ids:
            await hub.mark_removed(channel_id, vid)
            envelope = Envelope(
                channel_id=channel_id,
                sender_id=metadata.creator_id,
                audience=None,
