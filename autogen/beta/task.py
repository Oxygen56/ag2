...
class Task:
    ...
    def resumed_state(self) -> dict[str, Any] | None:
        """Prior checkpoint state if this Task was constructed with
        ``resume_from`` and a checkpoint existed; otherwise ``None``.
    ...
    async def checkpoint(self, state: dict[str, Any]) -> None:
        """Persist owner-supplied resume state.
    ...
    async def __aenter__(self) -> "Task":
        ...
        # When resuming from a prior task, read the checkpoint and pin
        # the new task's id to the resumed one so subsequent
        # ``checkpoint`` writes land on the same key. The store may
        # legitimately return ``None`` (checkpoint never written, or
        # store cleared) — in that case the resume reduces to a fresh
        # task that happens to share the prior id.
