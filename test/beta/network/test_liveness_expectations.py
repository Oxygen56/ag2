...
async def test_hide_handler_suppresses_notify_to_violator() -> None:
    ...
    clock.advance(120)
    await hub.evaluate_expectations()
    assert hub.is_hidden(session.channel_id, bob.agent_id)
...
async def test_remove_handler_bars_sender_and_persists(tmp_path) -> None:
    ...
    assert hub.is_removed(session.channel_id, bob.agent_id)
    ...
    body = await store.read(channel_removed_path(session.channel_id))
    ...
    assert hub2.is_removed(session.channel_id, bob.agent_id)
...
async def test_turn_within_fires_for_idle_workflow_speaker() -> None:
    ...
    custom.manifest = ChannelManifest(
