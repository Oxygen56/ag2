...
async def test_required_acks_activates_with_partial_quorum() -> None:
    ...
    # Drive create_channel directly so we can pin required_acks.
...
async def test_quorum_changed_fires_on_remove() -> None:
    ...
    await hub.mark_removed(metadata.channel_id, carol.agent_id)
    await asyncio.sleep(0.01)  # let the post_envelope finish

    wal = await hub.read_wal(metadata.channel_id)
...
async def test_quorum_changed_only_for_active_sessions() -> None:
    ...
    await hub.close_channel(session.channel_id, reason="manual")

    # Hub is closed; mark_removed shouldn't crash or emit on a
    # terminal session.
    await hub.mark_removed(session.channel_id, bob.agent_id)
