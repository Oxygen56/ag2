...
async def test_ack_advances_cursor_and_persists() -> None:
    ...
    wal = await hub.read_wal(session.metadata.channel_id)
    text_env = next(e for e in wal if e.event_type == EV_TEXT)

    # Cursor should now reflect bob's most recent successful ack — the
    # EV_TEXT envelope.
    cursor = hub._inbox_cursors.get(bob.agent_id, {}).get(session.metadata.channel_id)
    ...
    assert persisted[session.metadata.channel_id] == text_env.envelope_id
...
async def test_hydrate_restores_cursor() -> None:
    ...
    assert hub2._inbox_cursors[bob.agent_id][session.metadata.channel_id] == text_env.envelope_id
    await hub2.close()
...
async def test_nack_appends_to_inbox_nacks() -> None:
    ...
    assert record["channel_id"] == "sess-abc"
...
async def test_hello_replay_redelivers_unacked_envelopes() -> None:
    ...
    async with serve_ws(hub) as port:
        ...
        async def consume_until_text() -> None:
            async for frame in bob_ws1.frames():
                ...
                if isinstance(frame, NotifyFrame):
                    ev = frame.envelope
                    if ev.event_type == EV_CHANNEL_INVITE:
                        ack = Envelope(
        ...
        cursor_for_bob = hub._inbox_cursors.get(bob.agent_id, {}).get(session.metadata.channel_id)
