...
async def test_workflow_custom_text_contains_condition(beta_gemini_config) -> None:
    ...
    session_handle = Channel(metadata=session.metadata, client=writer)
    deps = {CHANNEL_DEP: session_handle, AGENT_CLIENT_DEP: writer, HUB_DEP: hub}
    ...
    async def _terminal() -> bool:
        state = hub._adapter_states.get(session.channel_id)
        if state is not None and state.expected_next_speaker is None:
            return True
        meta = await hub.get_channel(session.channel_id)
    ...
    final = await hub.get_channel(session.channel_id)
    if saw_approve:
