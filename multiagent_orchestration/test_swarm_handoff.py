...
async def test_workflow_swarm_handoff_routes_via_tool(beta_gemini_config) -> None:
    ...
    session_handle = Channel(metadata=session.metadata, client=triage)
    deps = {CHANNEL_DEP: session_handle, AGENT_CLIENT_DEP: triage, HUB_DEP: hub}
    ...
    # the same.)
    closed = await hub.close_channel(session.channel_id, reason="triage_done")
